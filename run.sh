...
function install {
    ...
    # python -m pip install cookiecutter pytest pre-commit --break-system-packages
...
function generate-project {
    ...
    git add --all
    git branch -M main
    git commit -m "feat: generated sample project with python_project_generator"
}
...
#    GITHUB_USERNAME - name of my github user, e.g. minhxuvi
...
EOF
    ...
    git commit -m 'feat!: populated from `python_project_generator` template'

    # if GH_TOKEN is set, set the remote url to it
    if [[ -n "$GH_TOKEN" ]]; then
        git remote set-url origin "https://$GITHUB_USERNAME:$GH_TOKEN@github.com/minhxuvi/$REPO_NAME"
    fi
    git push origin "$UNIQUE_BRANCH_NAME"

    # open a PR from the feature branch into main
    gh pr create \
        --title "feat: populated from \`python_project_generator\` template" \
        --body "This PR was generated by \`python_project_generator\`" \
        --base main \
